...
use spring_boot_app;
create table clientes (edad integer, estado bit, client_id bigint not null auto_increment, num_cedula bigint not null, direccion varchar(255), genero varchar(255), nombre varchar(255), password varchar(255), telefono varchar(255) not null, primary key (client_id)) engine=InnoDB;
...
alter table cuentas add constraint FK65yk2321jpusl3fk96lqehrli foreign key (cliente_id) references clientes (client_id);
